...
The two derivation kinds differ in how far the new post's stance moves from
...
Thread depth comes from reply chains, not from these: see
`dynamics/hawkes.py`, which generates reply posts from the self-exciting
thread intensity in the generation phase.
...
# spec §2.7: "A repost or quote creates a derived post". Reply *posts* are
# generated by the Hawkes draw in the generation phase (§2.3, §3.1 step 2);
# the kernel's `reply` action is an engagement event, not a cascade.
...
STANCE_SHIFT = {"repost": 0.0, "quote": 0.4}
