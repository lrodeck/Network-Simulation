...
|---|---|---|---|
| **C1 affect block** | Affective polarization — animus toward the other camp, attachment to one's own — rose while ideological positions moved little (Iyengar, Sood & Lelkes 2012; Iyengar & Westwood 2015); out-group animus is the strongest single engagement predictor measured (Rathje et al. 2021), motivating the block's EXISTENCE — but the `"distance"` drive mode it implements (`dynamics/drift.py::affect_delta`) is distance-graded escalation with an out-group PREMIUM, same-camp contact still contributing ~60-71% of what cross-camp contact does, not the out-group-SPECIFIC mechanism the Rathje citation itself reports (that finding is about sharing odds, grounding C5 below instead — FINDINGS.md, work order 01) | `population.affect`, `lr_affect`, `affect_ou_k` | off |
